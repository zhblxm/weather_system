          ...
          oLanguage: {  sLengthMenu: "每页_MENU_记录",    sInfo: "显示第_START_到_END_条，共_TOTAL_条",   
        	  ...
        	  oPaginate: { sFirst: "首页", sPrevious: "上一页",  sNext: "下一页",  sLast: "尾页"   }  },
               ...
               columns:  [
                          ...
                          {data: "invalidFieldDesc",  title: "要素名称",sortable: false },                     
                          {data: "minValue", title: "最小值",sortable: false },
                          {data: "value", title: "收到数据",sortable: false  },                     
                          {data: "maxValue", title: "最大值",sortable: false },
                          {data: "collectionDate", title: "日期",sortable: false }
                 ],
                 ...
                 "ajax": {
                     "url": "/invalid/stations",
                     type: 'GET',
                     "dataType": 'json'
                 }
	    });		
	  $('#invalidtable').addClass("table table-striped table-bordered form-inline dataTable no-footer");
	  var processParameterFun=function(){
		  var values=[],el=$('#parameters'),parametersValues=$('#parameters').val();
		  el.empty();
		  $.post("/invalid/parameters",{station:$('#station').val().join(",")},function(response,status){
	   ...
	   window.onload=function(){
		...
		$("#btnSearch").click(function() {
			...
			table.ajax.url("/invalid/stations?startdate=" + $("#startdate").val()+"&enddate="+ $("#enddate").val()+"&station="+$("#station").val().join()+"&parameters="+$("#parameters").val().join()).load();
		});
		$("#btnExport").click(function() {
			...
			if( $("#parameters").val().length>10)
			{
			 ...
			 window.open("/invalid/download?startdate=" + $("#startdate").val()+"&enddate="+ $("#enddate").val()+"&station="+$("#station").val().join()+"&parameters="+$("#parameters").val().join());
		});
})(jQuery);