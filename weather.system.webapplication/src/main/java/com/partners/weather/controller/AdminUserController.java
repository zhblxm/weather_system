...
public class AdminUserController {
    ...
    public ResponseMsg SaveUser(@RequestBody Adminuser adminuser) {
        ...
        if (StringUtils.isBlank(adminuser.getUniqueUserId()) && StringUtils.isBlank(adminuser.getUserPassword())) {
            responseMsg.setStatusCode(1);
            responseMsg.setMessage("密码不能为空。");
            return responseMsg;
        }

        int groupId = StringUtils.isNotBlank(adminuser.getUniqueGroupId()) ? HexUtil.HexToInt(adminuser.getUniqueGroupId()) : 0,
                userId = StringUtils.isNotBlank(adminuser.getUniqueUserId()) ? HexUtil.HexToInt(adminuser.getUniqueUserId()) : 0;
