	...
	public void run() {
		...
		try {
			...
			for (WeatherstationClient weatherstationClient : WeatherstationClients) {
				// 构建Terminalhistory查询条件
				/*
				VTerminalhistory vTerminalhistory = new VTerminalhistory();
				vTerminalhistory.setWeatherStationId(weatherstationClient.getTeminalParameterCategoryId());
				vTerminalhistory.setTerminalCatetoryId(weatherstationClient.getWeatherStationId());
				vTerminalhistory.setCreateDateFrom(now.minusDays(1).toString("yyyy-MM-dd 00:00:00"));
				vTerminalhistory.setCreateDateTo(now.toString("yyyy-MM-dd 00:00:00"));
				// 查找前一天所有Terminalhistory数据
				List<Terminalhistory> Terminalhistorys = TaskFactory.getInstance().getTerminalHistoryService()
						.getTerminalhistorys(vTerminalhistory);
				*/
