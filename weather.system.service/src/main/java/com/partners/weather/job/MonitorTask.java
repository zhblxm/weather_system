...
public class MonitorTask implements Runnable {
	...
	public void run() {
		...
		for (Weatherstation weatherstation : weatherstations) {
			if(!heartbeatMap.containsKey(weatherstation.getWeatherStationNumber()) || 
					now.compareTo(heartbeatMap.get(weatherstation.getWeatherStationNumber()).getCreateDate().plusMinutes(CommonResources.ONOROFFLINECOUNT)) > 0){
